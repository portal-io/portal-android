...
public class CancelReserve extends UseCase<CMSResponse, String> {

    public CancelReserve(IRepositoryManager repositoryManager, Scheduler executeThread, Scheduler postExecutionThread) {
                ...
                .map(new Function<UserModel, UserModel>() {
                    ...
                    public ObservableSource<CMSResponse> apply(@NonNull UserModel userModel) throws Exception {
                        return getRepositoryManager().obtainRemoteService(LiveAPI.class)
                                .cancel(userModel.getAccount_id(),userModel.getAccessTokenModel().getAccesstoken(),userModel.getDeviceId(), code);
                    }
