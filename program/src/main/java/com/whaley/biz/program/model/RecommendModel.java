...
public class RecommendModel extends BaseModel implements Parcelable, Serializable, ProgramConstants {
    ...
    public int getLiveStatus() {
        int status;
        try {
            status = Integer.valueOf(liveStatus);
        } catch (Exception e) {
            status = -1;
        }
        return status;
    ...
    public PlayData getPlayData() {
        ...
        if (linkArrangeType.equals(LINKARRANGETYPE_PROGRAM)
                || linkArrangeType.equals(LINKARRANGETYPE_LIVE)
                || linkArrangeType.equals(LINKARRANGETYPE_MORETVPROGRAM)
                || linkArrangeType.equals(LINKARRANGETYPE_MOREMOVIEPROGRAM)
                || linkArrangeType.equals(LINKARRANGETYPE_DYNAMICPROGRAM)) {
            int type;
            if (linkArrangeType.equals(LINKARRANGETYPE_PROGRAM)) {
                if (videoType.equals(VIDEO_TYPE_3D)) {
                    type = TYPE_PLAY_3D;
                } else {
                    type = TYPE_PLAY_PANO;
                }
            } else if (linkArrangeType.equals(LINKARRANGETYPE_LIVE)) {
                type = TYPE_PLAY_LIVE;
                isHorizontal = getDisplayMode() == 1;
            } else if (linkArrangeType.equals(LINKARRANGETYPE_MORETVPROGRAM)) {
                type = TYPE_PLAY_MORETV_TV;
            } else if (linkArrangeType.equals(LINKARRANGETYPE_MOREMOVIEPROGRAM)) {
                type = TYPE_PLAY_MORETV_2D;
            } else {
                type = TYPE_PLAY_PANO;
            }
            return DataBuilder.createBuilder(videoUrl, type)
                    .setId(linkArrangeValue)
                    .setTitle(name)
                    .setMonocular(true)
                    .putCustomData(PlayerDataConstants.LIVE_INIT_BACKGROUND_IMAGE, newPicUrl)
                    .putCustomData(PlayerDataConstants.ORIENTATION_IS_LANDSCAPE, isHorizontal)
                    .build();
        }
        return null;
    }
